...
module Generator
  class << self
    ...
    def empty_expectation_hash
      SpecForge::Normalizer.default(:expectation).except(:variables, *empty_request_hash.keys)
    end

    def forge(global: {}, metadata: {}, specs: [])
      specs = specs.map do |spec|
        default_spec = SpecForge::Normalizer.default(:spec)
        spec = default_spec.deep_merge(spec)

        spec[:id] = SecureRandom.uuid if spec[:id].blank?

        spec[:expectations].map! do |expectation|
          default_expectation = SpecForge::Normalizer.default(:expectation)
